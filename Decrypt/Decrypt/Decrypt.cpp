...
int main()
{
	...
	while (cin >> input)
	{
		...
		case 2:
		{
			...
			{
				First_word = ciphertext[i];
				Second_word = ciphertext[i + 1];

				for (int x = 0; x < 5; x++)
				{
					for (int y = 0; y < 5; y++)
					{
						if (table[x][y] == First_word) {
							First_index[0] = x;
							First_index[1] = y;
						}
						if (table[x][y] == Second_word) {
							Second_index[0] = x;
							Second_index[1] = y;
						}
					}
				}
				//cout <<"***"<< First_index[0]<<','<< First_index[1]<< " " << Second_index[0]<<','<< Second_index[1]<<endl;

				if (First_index[0] == Second_index[0]) {
					if (First_index[1] - 1 == -1) {
						First_index[1] = 4;
					}
					else {
						First_index[1] -= 1;
					}
					if (Second_index[1] - 1 == -1) {
						Second_index[1] = 4;
					}
					else {
						Second_index[1] -= 1;
					...
					if (First_index[0] - 1 == -1) {
						First_index[0] = 4;
					}
					else {
						First_index[0] -= 1;
					}
					if (Second_index[0] - 1 == -1) {
						Second_index[0] = 4;
					}
					else {
						Second_index[0] -= 1;
				...
				plaintext += table[First_index[0]][First_index[1]];
				plaintext += table[Second_index[0]][Second_index[1]];
			}
			cout << plaintext;
			break;
		}
